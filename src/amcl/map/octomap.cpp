...
void OctoMap::initFromOctree(std::shared_ptr<octomap::OcTree> octree, double max_occ_dist)
{
  ...
  max_occ_dist_ratio_ = max_occ_dist_ / DTYPE_MAX;
...
void OctoMap::updateCSpace()
{
  ...
  distance_ratios_.resize(num_z_column_indices_, DTYPE_MAX);
...
void OctoMap::setOccDist(int i, int j, int k, double d)
{
  ...
  if(distances_start_index == 0)
  {
    ...
    distance_ratios_.resize(distances_start_index + num_z_column_indices_, DTYPE_MAX);
  }
  uint8_t distance_ratio = std::min(DTYPE_MAX, static_cast<int>(std::floor(d / max_occ_dist_ * (DTYPE_MAX + 1))));
