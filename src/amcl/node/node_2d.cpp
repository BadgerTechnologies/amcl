...
bool Node2D::isMapInitialized()
{
  ...
  if (not map_->isDistancesCreated())
  {
...
bool Node2D::initFrameToScanner(const std::string& frame_id, tf2::Transform* scanner_pose, int* scanner_index)
{
  ...
  *scanner_index = frame_to_scanner_.size();
  ...
  try
  {
    geometry_msgs::TransformStamped t = tf_buffer_.lookupTransform(frame_id, node_->getBaseFrameId(),
...
bool Node2D::getAngleStats(const sensor_msgs::LaserScanConstPtr& planar_scan, double* angle_min, double* angle_increment)
{
  ...
  try
  {
    geometry_msgs::TransformStamped t = tf_buffer_.lookupTransform(planar_scan->header.frame_id, node_->getBaseFrameId(),
